...
void ALabyrinthe::genererLabyrinthe() {
	...
	for (int x = 0; x < 10; x++)
	{
		...
		for (int y = 0; y < 10; y++)
		{
			...
			listeCellules[x][y] = monde->SpawnActor<ACellule>(this->objetApparition, FVector((x - 4.5f) * 1000, (y - 4.5f) * 1000, 0), rotation, parametresApparition);
			listeCellules[x][y]->setCoordonnee(x, y);
		}
	}
	...
	listeCellules[0][0]->ouvrirCellule();
